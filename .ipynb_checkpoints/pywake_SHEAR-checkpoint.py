...
if __name__ == '__main__':
    ...
    for j in list_j :
        for i in ['00', '03', '06', '09', '12', '15', '18', '21'] :
            ...
            h_WF = xr.open_dataset(data_folder + WFname + '/withOWF/crop'+fname+'.nc').squeeze() # Path to data
            ...
            if 'pres' in h_WF.data_vars :
                h_WF = h_WF.drop_vars(['pres'])
            if 'u' in h_WF.data_vars :
                h_WF = h_WF.drop_vars(['u', 'v'])
            if 'height_4' in h_WF.dims :
                h_WF = h_WF.drop_dims(['height_4'])
            if 'height_2' in h_WF.dims :
                h_WF = h_WF.drop_dims(['height_2'])
            ...
            if mean_ratio <= 1.0 and mean_ratio >= 0.6 : # Medium ratio: <=0.6 and >=1.0 ; High ratio: <1.0 ; Low ratio >0.6) # IF VERTICAL SHEAR IS IN RANGE
                ...
                file = output_dir + region + '_area_' + WFname + '_' + fname.split('/')[-1] + '.nc' # Change name here #####################################################################
